...
def tpu_compatible_files(filenames):
    """Register a validator to check directory flags.
    Args:
        filenames: An iterable of strings containing the names of flags to be
        checked.
    """
    valid = True
    for filename in filenames:
        if not filename.startswith("gs://"):
            log.error(f"TPU requires GCS path. Non GCS path provided: {filename}.")
            valid = False
